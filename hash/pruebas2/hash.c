...
#define FACTOR_CARGA_MAX 0.8
#define FACTOR_CARGA_MIN 0.1
#define CAPACIDAD_MENOR 0.8
...
//https://en.wikipedia.org/wiki/Jenkins_hash_function (proporcionada por nuestro corrector. Gracias Tomi!)
...
// pre: la tabla fue creada, el largo esta inicializado
// post: destruye la tabla y todo lo que contiene
...
bool hash_pertenece(const hash_t *hash, const char *clave){
    if((posicion_clave(hash, clave) == (size_t)NO_ENCONTRADO)) return false;
    return true;
}


void *hash_obtener(const hash_t *hash, const char *clave){
    size_t posicion = posicion_clave(hash, clave);
    if(posicion != (size_t)NO_ENCONTRADO)return hash->tabla[posicion].dato;
    return NULL;
}
...
void *hash_borrar(hash_t * hash, const char *clave){
    size_t posicion = posicion_clave(hash, clave);
    if (posicion == (size_t)NO_ENCONTRADO) return NULL;
